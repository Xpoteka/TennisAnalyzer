...
dropping the same file again resumes it, and it is moved into the library once complete.
"""
...
VIDEO_SUFFIXES = {".mp4", ".mov", ".m4v", ".mkv", ".avi", ".mts", ".webm"}
...
def list_videos(data_root: Path, used_by: dict[Path, list[int]]) -> dict[str, Any]:
    """Every video in the library, which sessions use it, and uploads still in progress.

    ``used_by`` maps a resolved video path to the IDs of the sessions that include it.
    """
...
def delete_video(data_root: Path, raw_name: str) -> Path:
    """Remove one video from the library and return its path. Results made from it stay."""
    name = safe_name(raw_name)
