...
**Status: v2 in progress** (branch `v2`). Working: uploads, the job queue, playback, court and camera, player and ball tracking, hits, strokes, ball speed/height/placement, training vs match, and score keeping (point winners are still unreliable). Next: player profiles across sessions with technique analysis, and several videos per session. Accuracy on a labelled match is in [ARCHITECTURE.md](ARCHITECTURE.md).
